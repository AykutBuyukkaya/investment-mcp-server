...
Python MCP server for investment workflows. The current server exposes BIST market data tools backed by Yahoo Finance, using `.IS` ticker normalization for Turkish stock exchange symbols, plus gold price data backed by direct Canli Doviz provider calls.
...
- `get_ohlcv_bars`: fetches OHLCV candles for a BIST symbol over a requested Istanbul-time date range.
